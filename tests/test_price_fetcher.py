import pytest
from unittest.mock import patch, MagicMock # pytest-mock provides the mocker fixture, but unittest.mock is often used directly too
import requests
...
ERROR_RESPONSE_DATA_NO_CURRENCY = {
    "bitcoin": {}
}
...
@patch('src.price_fetcher.app_settings', spec=AppConfig) # Patch app_settings in price_fetcher.py
@patch('requests.get')
def test_get_crypto_price_success(mock_requests_get, mock_app_settings_instance, mock_app_settings_values):
    ...
    mock_requests_get.assert_called_with(
        mock_app_settings_values["coingecko_api_url"],
        params={"ids": "bitcoin", "vs_currencies": "usd"},
        timeout=mock_app_settings_values["api_request_timeout"]
    )

@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_id_not_found_in_response(mock_requests_get, mock_app_settings_instance, mock_app_settings_values):
    """Test when crypto ID is not in the API response."""
    mock_app_settings_instance.coingecko_api_url = mock_app_settings_values["coingecko_api_url"]
    mock_app_settings_instance.retry_max_retries = 1 # No retry needed here
    mock_app_settings_instance.api_request_timeout = mock_app_settings_values["api_request_timeout"]
    # Ensure other retry params are set if they were accessed by the code under test, even if not used for this path
    mock_app_settings_instance.retry_initial_backoff = mock_app_settings_values["retry_initial_backoff"]
    mock_app_settings_instance.retry_backoff_factor = mock_app_settings_values["retry_backoff_factor"]
    mock_app_settings_instance.retryable_status_codes = mock_app_settings_values["retryable_status_codes"]
...
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_currency_not_found_in_response(mock_requests_get, mock_app_settings_instance, mock_app_settings_values):
...
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_http_error_retry_and_fail(mock_requests_get, mock_app_settings_instance, mock_time_sleep, mock_app_settings_values):
    ...
    assert mock_time_sleep.call_count == mock_app_settings_values["retry_max_retries"] - 1

@patch('src.price_fetcher.time.sleep')
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_http_error_non_retryable(mock_requests_get, mock_app_settings_instance, mock_time_sleep, mock_app_settings_values):
    """Test a non-retryable HTTP error (e.g., 404)."""
    mock_app_settings_instance.coingecko_api_url = mock_app_settings_values["coingecko_api_url"]
    mock_app_settings_instance.retry_max_retries = mock_app_settings_values["retry_max_retries"]
    mock_app_settings_instance.retryable_status_codes = mock_app_settings_values["retryable_status_codes"]
    mock_app_settings_instance.api_request_timeout = mock_app_settings_values["api_request_timeout"]
    mock_app_settings_instance.retry_initial_backoff = mock_app_settings_values["retry_initial_backoff"]
    mock_app_settings_instance.retry_backoff_factor = mock_app_settings_values["retry_backoff_factor"]
...
@patch('src.price_fetcher.time.sleep') # Added sleep mock as it might be called if retries > 1
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_connection_error_retry_and_fail(mock_requests_get, mock_app_settings_instance, mock_time_sleep, mock_app_settings_values):
...
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_invalid_json(mock_requests_get, mock_app_settings_instance, mock_app_settings_values):
...
@patch('src.price_fetcher.app_settings', spec=AppConfig)
@patch('requests.get')
def test_get_crypto_price_retry_then_success(mock_requests_get, mock_app_settings_instance, mock_time_sleep, mock_app_settings_values):
    ...
    mock_time_sleep.assert_called_once_with(mock_app_settings_values["retry_initial_backoff"])

# Test for when app_settings is None (config file failed to load)
@patch('src.price_fetcher.app_settings', None) # Set app_settings to None in the price_fetcher module
@patch('requests.get') 
